    ...
    try:
        ...
        while True: 
            try: 
                ...
                time.sleep(2)
                continue
    sql_count = 'select max(id) from yuhuangshan.blackmetal_monitor;'
    try:
        cur = conn.cursor()   
...
def monitor_write2():       
    ...
    try:   
        ...
        cur.execute('select * from yuhuangshan.blackmetal_monitor order by id desc limit 1;')
        result3 = cur.fetchone()
        if result3 != None:  # 防止数据表格为空的情况
            result3 = map(str, result3)
            values = ','.join(result3[1:4])
            if result1 == result2:
                if flag == True:
                    title = '黑色金属输出风险：程序可能停止运行'
                    content = '\n预测输出程序可能停止运行，请确认是否正常。\n最后输出数据为：\n\"%s\"。\n最后输出时间为：\n\"%s\"。\n\n如需运行，请重新启动。\n如无需运行，请忽略此封邮件。\n\n\n如有疑问，请咨询:18662581318' % (values,result3[4]) 
                    print content
                    content = str(content)
